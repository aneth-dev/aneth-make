...
'''
NAME
    latex2pdf - Converts LaTeX source to PDF file

SYNOPSIS
    latex2pdf [options] INFILE

DESCRIPTION
    This filter reads LaTeX source text from the input file
    INFILE (or stdin if INFILE is -) and renders it to PDF file.
    Typically used to render math equations.

    Requires xelatex(1) commands and LaTeX math packages.
...
OPTIONS
    ...
    -m
        Skip if the PDF output file is newer that than the INFILE.
...
AUTHOR
    Modified by Thomas Pérennou, <thomas.perennou@aeten.net>
    from PNG version written by Stuart Rackham, <srackham@gmail.com>
    which code was inspired by Kjell Magne Fauske's code:
...
COPYING
    ...
    Copyright (C) 2016 Thomas Pérennou (PDF version from PNG).
'''
...
def latex2pdf(infile, outfile, modified):
    '''Convert LaTeX input file infile to PDF file named outfile.'''
    ...
    os.chdir(outdir)
    try:
        # Compile LaTeX document to PDF file.
        run('xelatex %s' % texfile)
        os.rename(basefile + '.pdf', outfile)
    finally:
        os.chdir(saved_pwd)
        for f in temps:
            if os.path.isfile(f):
...
def main():
    ...
    for o,v in opts:
        if o in ('--help','-h'):
            ...
            sys.exit(0)
        if o =='--version':
            print('latex2pdf version %s' % (VERSION,))
            sys.exit(0)
        if o == '-o': outfile = v
    ...
    # Do the work.
    latex2pdf(infile, outfile, modified)
